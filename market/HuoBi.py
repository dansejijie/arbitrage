...
from utils.MarketHelper import okexFromatTicker,okexFromatDepth,okexFromatOrder,okexFromatUserInfo,huobiFormatKLine,formatPeriod
...
class HuoBi(MarketClient):
    ...
    def kline(self,symbol,type='15min',size=2000,timeout=10):
        type=formatPeriod("huobi",type)
        temp=symbol.split('_')
        symbol_huobi=temp[0]+temp[1]
        url=kline_url+'?symbol=' + symbol_huobi + '&period=' + type+'&size='+str(size)
        try:
            r=requests.get(url,timeout=timeout)
            r=r.json()
            return huobiFormatKLine(symbol,r)
        except Exception as e:
            kline=KLine()
...
if __name__ == "__main__":
    ...
    market=HuoBi('29e388ed-8745-4820-bd2e-4b0921c68999','86076009C465CDB7A5C438F863515B3B')
